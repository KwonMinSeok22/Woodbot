...
function stations(client, prefix, message) {
  try {
    let amount = 0;
    const stationsembed = new Discord.MessageEmbed().setColor(ee.color).setFooter(ee.footertext, ee.footericon).setTitle("올바른 'Index' 번호를 입력하여 방송국을 선택해주세요!").setDescription("Example: `?radio 11`");
    const stationsembed2 = new Discord.MessageEmbed().setColor(ee.color).setFooter(ee.footertext, ee.footericon).setTitle("올바른 'Index' 번호를 입력하여 방송국을 선택해주세요!").setDescription("Example: `?radio 69`");
    const stationsembed3 = new Discord.MessageEmbed().setColor(ee.color).setFooter(ee.footertext, ee.footericon).setTitle("올바른 'Index' 번호를 입력하여 방송국을 선택해주세요!").setDescription("Example: `?radio 180`");
    let United_Kingdom = "";
    ...
    stationsembed.addField("🇬🇧 영국", `>>> ${United_Kingdom}`, true);
    amount++;
    let austria = "";
    ...
    stationsembed.addField("🇦🇹 오스트리아", `>>> ${austria}`, true);
    amount++;
    let Belgium = "";
    ...
    stationsembed.addField("🇧🇪 벨기에", `>>> ${Belgium}`, true);
    amount++;
    let Bosnia = "";
    ...
    stationsembed.addField("🇧🇦 보스니아", `>>> ${Bosnia}`, true);
    amount++;
    let Czech = "";
    ...
    stationsembed.addField("🇨🇿 체코", `>>> ${Czech}`, true);
    amount++;
    let Denmark = "";
    ...
    stationsembed.addField("🇩🇰 덴마크", `>>> ${Denmark}`, true);
    amount++;
    let germany = "";
    ...
    stationsembed2.addField("🇩🇪 독일", `>>> ${germany}`, true);
    amount++;
    let Hungary = "";
    ...
    stationsembed2.addField("🇭🇺 헝가리", `>>> ${Hungary}`, true);
    amount++;
    let Ireland = "";
    ...
    stationsembed2.addField("🇮🇪 아일랜드", `>>> ${Ireland}`, true);
    amount++;
    let Italy = "";
    ...
    stationsembed2.addField("🇮🇹 이탈리아", `>>> ${Italy}`, true);
    amount++;
    let Luxembourg = "";
    ...
    stationsembed2.addField("🇱🇺 룩셈부르크", `>>> ${Luxembourg}`, true);
    amount++;
    let Romania = "";
    ...
    stationsembed2.addField("🇷🇴 루마니아", `>>> ${Romania}`, true);
    amount++;
    let Serbia = "";
    ...
    stationsembed3.addField("🇷🇸 세르비아", `>>> ${Serbia}`, true);
    amount++;
    let Spain = "";
    ...
    stationsembed3.addField("🇪🇸 스페인", `>>> ${Spain}`, true);
    amount++;
    let Sweden = "";
    ...
    stationsembed3.addField("🇸🇪 스웨덴", `>>> ${Sweden}`, true);
    amount++;
    let Ukraine = "";
  ...
  try {
    ...
    if (!response || response.loadType === 'LOAD_FAILED' || response.loadType !== 'PLAYLIST_LOADED') {
      let embed = new MessageEmbed()
        .setTitle("❌ 오류 | 최신 곡과 관련된 내용을 찾을 수 없습니다!")
  ...
  try {
    ...
    function reset(track_info_msg, queue_info_msg) {
      let embed2 = new MessageEmbed()
        .setColor(ee.color)
        .setFooter(`현재 접두사:   ${client.settings.get(track_info_msg.guild.id, "prefix")}`, ee.footericon)
        .setTitle("Ocean | Music Queue")
        .setImage("https://cdn.discordapp.com/icons/750616868548509697/b90edf1f77b6a66b2bd8a3dd28aadb29.png?size=128")
        .setDescription(`Empty\nJoin a voice channel and queue songs by name or url in here.`)
      let embed3 = new MessageEmbed()
        .setColor(ee.color)
        .setFooter(ee.footertext, ee.footericon)
        .setTitle("Ocean | 현재 재생 중인 노래가 없습니다!")
        .setDescription(`> 음성 채널에 참가하고 재생할 노래 이름 또는 URL을 입력하십시오.\n\n[Invite Ocean - 관리자 권한 필요](https://discord.com/api/oauth2/authorize?client_id=793874255741714473&permissions=8&redirect_uri=http%3A%2F%2Flocalhost%3A8080%2Fcallback&scope=bot) • [Support Server](https://discord.gg/3EyfukUaXu)`)
        .setImage("https://cdn.discordapp.com/icons/750616868548509697/b90edf1f77b6a66b2bd8a3dd28aadb29.png?size=128")
    ...
    function SongEmbed(track) {
      ...
      let embed = new MessageEmbed()
        ...
        embed.setTitle(`⌛️ 진행률:`)
        embed.setDescription(`${createBarlul(player)}`)
        embed.addField("💯 **출처:**", `\`${track.author}\``, true)
        embed.addField(`⏯ **정보**`, `\`${player.playing ? `${emoji.msg.resume}  재생중` : `${emoji.msg.pause}  일시정지`}\``, true)
        embed.addField(`🩸 **요청인:**`, `${track.requester}`, true)
        embed.setThumbnail(`https://img.youtube.com/vi/${track.identifier}/mqdefault.jpg`)
        embed.setFooter(`재생목록: ${Player.queue.size}  •  볼륨: ${Player.volume}%  •  자동재생: ${Player.get(`autoplay`) ? `✔️` : `❌`}  •  반복: ${Player.queueRepeat ? `✔️` : Player.trackRepeat ? `✔️` : `❌`}`, Player.queue.current.requester.displayAvatarURL({
          dynamic: true
        }))
      return embed;
    }
    function QueueEmbed(client, player) {
      ...
      const embed = new MessageEmbed().setAuthor(`Ocean Music | Music Queue`);
      const multiple = 15;
      const page = 1;
      const end = page * multiple;
      const start = end - multiple;
      const tracks = queue.slice(start, end);
      if (queue.current) embed.addField("**0) 재생중인 노래**", `${queue.current.title.split("[").join("\[").split("]").join("\]").substr(0, 60)} [${track.isStream ? "LIVE STREAM" : format(track.duration).split(" | ")[0]}]\n요청인: ${queue.current.requester}`);
      if (!tracks.length) embed.setDescription(`재생중인 노래가 없습니다. ${page > 1 ? `페이지 ${page}` : "재생목록"}.`);
      else embed.setDescription(tracks.map((track, i) => `**${start + ++i})** ${track.title.split("[").join("\[").split("]").join("\]").substr(0, 60)} [${track.isStream ? "LIVE STREAM" : format(track.duration).split(" | ")[0]}]\n요청인: ${track.requester}`).join("\n"));
   embed.setColor(ee.color);
      embed.setFooter(ee.footertext, ee.footericon);
      return embed;
    }
...
async function edit_request_message_queue_info(client, player) {
 ...
 embed.setColor(ee.color);
    ...
    track_info_msg.edit(oldembed.setFooter(`재생목록: ${player.queue.size}  •  볼륨: ${player.volume}%  •  자동재생: ${player.get(`autoplay`) ? `✔️` : `❌`}  •  반복: ${player.queueRepeat ? `✔️ 재생목록` : player.trackRepeat ? `✔️ 노래` : `❌`}`, player.queue.current.requester.displayAvatarURL({
      dynamic: true
